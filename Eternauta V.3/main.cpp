...
void setup() {
    // Guardar el estado actual del arma y casco antes de eliminar a Juan Salvo
    Eternauta::Arma arma_actual = (juanSalvo != nullptr) ? juanSalvo->getArmaActual() : Eternauta::Arma::NINGUNA;
    bool tiene_casco = (juanSalvo != nullptr) ? juanSalvo->tieneCasco() : false;
    
    delete juanSalvo;
    juanSalvo = new Eternauta(ANCHO/2, ALTO-5);
    
    // Restaurar el arma y casco si se cambia de nivel
    if (nivel > 1) {
        if (arma_actual != Eternauta::Arma::NINGUNA) {
            juanSalvo->recogerObjeto((arma_actual == Eternauta::Arma::PALO) ? Objeto::Tipo::PALO :
                                    (arma_actual == Eternauta::Arma::REVOLVER) ? Objeto::Tipo::REVOLVER :
                                    Objeto::Tipo::ESCOPETA);
        }
        if (tiene_casco) {
            juanSalvo->recogerObjeto(Objeto::Tipo::CASCO);
        }
    }
    ...
    if (nivel == 1) {
        ...
        // Solo aparecen armas si Juan no tiene ninguna
        if (arma_actual == Eternauta::Arma::NINGUNA) {
        ...
        // No aparecen armas en nivel 3, se conservan las del nivel anterior
    }
}
...
void update() 
{
    ...
    // Aparecer el casco después de 10 segundos (200 frames)
    if (nivel == 1 && tiempo_juego == 200 && objetos.empty()) {
        Objeto casco(rand() % (ANCHO-4) + 2, 5, Objeto::Tipo::CASCO);
        objetos.push_back(casco);
    }
    
    juanSalvo->update();
    
    // Verificar transición nivel 1 -> 2
...
for (auto& bala : balasEnemigas) 
{
    ...
    {
        // Para cascarudos (nivel 2)
        if (nivel == 2) 
        {
            for (auto& cascarudo : cascarudos) 
            {
                if (cascarudo.colisionCon(bala.getX(), bala.getY())) 
                {
                    cascarudo.recibirDano(juanSalvo->getArmaActual());
                    bala.setActiva(false);
                    
                    if (cascarudo.estaMuerto()) 
                    {
                        puntaje += 100;
                    }
                    break; // Salir del loop de cascarudos
                }
            }
        }
        // Para humanos controlados (nivel 3)
        else if (nivel == 3) 
        {
            for (auto& humano : humanos) 
            {
                if (humano.colisionCon(bala.getX(), bala.getY())) 
                {
                    humano.recibirDano();
                    bala.setActiva(false);
                    ...
                    break; // Salir del loop de humanos
                }
            }
        }
    }
    ...
    if (nivel == 2) {
        for (auto& cascarudo : cascarudos) {
            ...
            for (auto& bala : balasEnemigas) {
                if (bala.getTipo() == Bala::Tipo::PROTAGONISTA && 
                    cascarudo.colisionCon(bala.getX(), bala.getY())) {
                    cascarudo.recibirDano(juanSalvo->getArmaActual());
                    bala.setActiva(false);
            ...
            if (juanSalvo->colisionCon(cascarudo.getX(), cascarudo.getY())) {
                juanSalvo->recibirDano();
            }
        }
        
        // Eliminar cascarudos muertos
        cascarudos.erase(
            remove_if(cascarudos.begin(), cascarudos.end(), 
                     [](Cascarudo& c) { return c.estaMuerto(); }),
            cascarudos.end()
        );
        
        // Verificar transición nivel 2 -> 3
...
void draw() {
    ...
    switch (juanSalvo->getArmaActual()) {
        case Eternauta::Arma::NINGUNA: mvprintw(0, 71, "NINGUNA"); break;
        case Eternauta::Arma::PALO: mvprintw(0, 71, "PALO"); break;
        case Eternauta::Arma::REVOLVER: mvprintw(0, 71, "REVOLVER"); break;
        case Eternauta::Arma::ESCOPETA: mvprintw(0, 71, "ESCOPETA"); break;
    }
    if (juanSalvo->tieneCasco()) mvprintw(0, 85, "CASCO: SI");
    else mvprintw(0, 85, "CASCO: NO");
    
    // Info debug para cascarudos
    if (nivel == 2) {
        mvprintw(1, 2, "CASCARUDOS: %d", cascarudos.size());
    }
    
    // Dibujar elementos
    if (nivel == 1) for (auto& copo : coposNieve) copo.draw();
    else if (nivel == 2) for (auto& cascarudo : cascarudos) cascarudo.draw();
    else if (nivel == 3) for (auto& humano : humanos) humano.draw();
    
    for (auto& bala : balasEnemigas) bala.draw();
    for (auto& objeto : objetos) objeto.draw();
    juanSalvo->draw();
...
void mostrarMenu() {
    ...
    switch (opcion) {
        case 1: nivel = 1; estado_juego = JUGANDO; break;
        case 2: estado_juego = SALIR; break;
    }
}
...
void mostrarGameOver() {
    ...
    int tecla = getch();
    if (tecla == 's' || tecla == 'S') estado_juego = MENU_PRINCIPAL;
    else estado_juego = SALIR;
}

void mostrarNivelCompletado() {
    pantallas.mostrarNivelCompletado(nivel, puntaje);
    if (nivel < 3) cambiarNivel(nivel + 1);
    else {
