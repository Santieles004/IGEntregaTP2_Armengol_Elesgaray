...
void Eternauta::disparar(std::vector<Bala>& balas) {
    if (m_arma != Arma::NINGUNA && m_estado != Estado::MUERTO) {
        ...
        if (m_arma == Arma::ESCOPETA) {
            // RAFAGA DE 5 BALAS para la escopeta
            for (int i = -2; i <= 2; i++) {
                balas.push_back(Bala(m_x + 1 + i, m_y - 1, Bala::Tipo::PROTAGONISTA, -velocidad));
            }
        } else {
            // Disparo normal para revolver y palo
...
void Eternauta::recogerObjeto(Objeto::Tipo tipo) {
    switch (tipo) {
        case Objeto::Tipo::PALO:
            m_arma = Arma::PALO;
            std::cout << "¡AGARRASTE LA PALANCA!" << std::endl;
            break;
        case Objeto::Tipo::REVOLVER:
            m_arma = Arma::REVOLVER;
            std::cout << "¡AGARRASTE EL REVOLVER!" << std::endl;
            break;
        case Objeto::Tipo::ESCOPETA:
            m_arma = Arma::ESCOPETA;
            std::cout << "¡AGARRASTE LA ESCOPETA!" << std::endl;
            break;
